...
function StarRow({ rating, size = 14 }: { rating: number; size?: number }) {
  return (
    <View className="flex-row gap-0.5">
      {[1, 2, 3, 4, 5].map((i) => (
        <Star key={i} size={size} color="#ff4b26" fill={i <= rating ? "#ff4b26" : "transparent"} />
      ))}
    </View>
  );
}
    ...
    <View className="flex-row gap-2 justify-center">
      {[1, 2, 3, 4, 5].map((i) => (
        <TouchableOpacity key={i} onPress={() => onChange(i)}>
          <Star size={32} color="#ff4b26" fill={i <= rating ? "#ff4b26" : "transparent"} />
        </TouchableOpacity>
      ))}
    </View>
  );
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50">
      ...
      <ScrollView className="flex-1 px-5 pt-5" showsVerticalScrollIndicator={false}>
        {loading ? (
          <View className="items-center justify-center pt-20">
            ...
            <Text className="font-manrope text-sm text-gray-400 mt-3">Loading reviews…</Text>
          </View>
        ) : reviews.length === 0 ? (
          <EmptyState
            Icon={Star}
            ...
            {reviews.map((r) => (
              <View key={r.id} className="bg-white rounded-3xl p-5" style={shadow.md}>
                ...
                <View className="flex-row items-center gap-2 mb-3">
                  ...
                  <Text className="text-sm font-grotesk-bold text-gray-900 flex-1" numberOfLines={1}>
                ...
                <View className="flex-row items-center justify-between mb-3">
                  ...
                  <Text className="font-manrope text-[10px] text-gray-400">{formatDate(r.created_at)}</Text>
                </View>

                {/* Comment */}
                {r.comment && (
                  <Text className="font-grotesk text-sm text-gray-600 leading-relaxed mb-4">{r.comment}</Text>
                )}
                ...
                <View className="flex-row gap-2 pt-3 border-t border-gray-100">
                  ...
                  >
                    ...
                    <Text className="text-xs font-grotesk-bold text-blue-600">Edit</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                  ...
                  >
                    ...
                    <Text className="text-xs font-grotesk-bold text-red-600">Delete</Text>
                  </TouchableOpacity>
                </View>
              </View>
            ))}
      ...
      <Modal visible={!!editTarget} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setEditTarget(null)}>
        <SafeAreaView className="flex-1 bg-white">
          <View className="flex-row items-center justify-between px-5 pt-4 pb-4 border-b border-gray-100">
            <Text className="text-lg font-grotesk-extrabold text-gray-900">Edit Review</Text>
          ...
          <ScrollView className="flex-1 px-5 pt-6" showsVerticalScrollIndicator={false}>
            <View className="gap-6 pb-10">
              <View className="items-center gap-3">
                <Text className="text-sm font-grotesk-semibold text-gray-700">Your Rating</Text>
              ...
              <View className="gap-2">
                <Text className="text-sm font-grotesk-semibold text-gray-700">Your Review (optional)</Text>
                <TextInput
              ...
              >
                {saving
                  ...
                  : <Text className="text-white font-grotesk-bold">Save Review</Text>}
              </TouchableOpacity>
            </View>
          </ScrollView>
        </SafeAreaView>
      ...
      <Modal visible={showNewModal} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setShowNewModal(false)}>
        <SafeAreaView className="flex-1 bg-white">
          <View className="flex-row items-center justify-between px-5 pt-4 pb-4 border-b border-gray-100">
            <Text className="text-lg font-grotesk-extrabold text-gray-900">Leave a Review</Text>
            ...
            <View className="gap-6 pb-10">
              {newProductName ? (
                <View className="flex-row items-center gap-2 bg-gray-50 rounded-xl p-3">
                  <Package size={16} color="#6b7280" />
                  <Text className="text-sm font-grotesk-bold text-gray-800 flex-1" numberOfLines={1}>{newProductName}</Text>
                </View>
              ) : null}
              <View className="items-center gap-3">
                <Text className="text-sm font-grotesk-semibold text-gray-700">Your Rating</Text>
                <View className="flex-row gap-2 justify-center">
                  {[1, 2, 3, 4, 5].map((i) => (
                    <TouchableOpacity key={i} onPress={() => setNewRating(i)}>
                      <Star size={32} color="#ff4b26" fill={i <= newRating ? "#ff4b26" : "transparent"} />
                    </TouchableOpacity>
                  ))}
                </View>
                <Text className="font-manrope text-xs text-gray-400">
              ...
              <TouchableOpacity
                ...
                style={shadow.btn}
              >
                {savingNew ? <ActivityIndicator color="#fff" /> : <Text className="text-white font-grotesk-bold">Submit Review</Text>}
              </TouchableOpacity>
            </View>
          </ScrollView>
        </SafeAreaView>
