      ...
      <SafeAreaView className="flex-1 bg-gray-50">
        ...
        <View className="px-5">
          <TouchableOpacity
            ...
            style={shadow.btn}
          >
            <Text className="text-white font-grotesk-bold">Apply Now</Text>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
    );
      ...
      <ScrollView className="flex-1 px-5 pt-5" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
        <View className="gap-5">
          <View className="flex-row items-center justify-between bg-white rounded-2xl p-4" style={shadow.md}>
            <Text className="text-sm font-grotesk-bold text-gray-700">Status</Text>
          ...
          {myApplication.status === "rejected" && myApplication.decision_reason && (
            <View className="bg-red-50 rounded-2xl p-4 border border-red-100">
              <Text className="text-xs font-grotesk-bold text-red-700 mb-1">Reason</Text>
              <Text className="font-grotesk text-sm text-red-700 leading-relaxed">{myApplication.decision_reason}</Text>
            </View>
          )}
          ...
          {myApplication.status === "revoked" && myApplication.revocation_reason && (
            <View className="bg-red-50 rounded-2xl p-4 border border-red-100">
              <Text className="text-xs font-grotesk-bold text-red-700 mb-1">Reason</Text>
              <Text className="font-grotesk text-sm text-red-700 leading-relaxed">{myApplication.revocation_reason}</Text>
            </View>
          )}

          <View className="bg-white rounded-2xl p-4 gap-4" style={shadow.md}>
            <Text className="text-sm font-grotesk-bold text-gray-700">Application Details</Text>
            <View className="flex-row flex-wrap gap-4">
              <View className="flex-1 min-w-[45%]">
                <Text className="text-[10px] text-gray-400 font-grotesk-semibold uppercase mb-1">Employment Status</Text>
                <Text className="text-sm font-grotesk-bold text-gray-900 capitalize">{myApplication.employment_status.replace("_", " ")}</Text>
              </View>
              <View className="flex-1 min-w-[45%]">
                <Text className="text-[10px] text-gray-400 font-grotesk-semibold uppercase mb-1">Monthly Income</Text>
                <Text className="text-sm font-grotesk-bold text-gray-900">{fmt(myApplication.monthly_income)}</Text>
              </View>
              {myApplication.employer_name && (
                <View className="flex-1 min-w-[45%]">
                  <Text className="text-[10px] text-gray-400 font-grotesk-semibold uppercase mb-1">Employer</Text>
                  <Text className="text-sm font-grotesk-bold text-gray-900">{myApplication.employer_name}</Text>
                </View>
              )}
              {myApplication.job_title && (
                <View className="flex-1 min-w-[45%]">
                  <Text className="text-[10px] text-gray-400 font-grotesk-semibold uppercase mb-1">Job Title</Text>
                  <Text className="text-sm font-grotesk-bold text-gray-900">{myApplication.job_title}</Text>
                </View>
              )}
            </View>
            ...
            {myApplication.documents.length > 0 && (
              <View className="gap-2 pt-2 border-t border-gray-100">
                <Text className="text-[10px] text-gray-400 font-grotesk-semibold uppercase mb-1">Documents</Text>
                {myApplication.documents.map((doc) => (
                  ...
                  >
                    <FileCheck2 size={14} color="#ff4b26" />
                    <Text className="text-sm text-[#e03f1c] font-grotesk-semibold">
          ...
          {canReapply && (
            <TouchableOpacity
              ...
              style={shadow.btn}
            >
              <Text className="text-white font-grotesk-bold">Apply Again</Text>
            </TouchableOpacity>
          )}
        </View>
      </ScrollView>
