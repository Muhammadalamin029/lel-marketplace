...
export default function EditProfileScreen() {
  ...
  const [form, setForm] = useState({ name: "Alameen Muhammad", email: "alameen@example.com", phone: "+234 800 000 0000", bio: "" });
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      // TODO: PATCH /auth/profile with form data
          ...
          <View className="items-center">
            <View className="relative">
              <View className="w-24 h-24 rounded-full bg-indigo-900 items-center justify-center">
                <Text className="text-white text-3xl font-bold">{form.name.charAt(0)}</Text>
              </View>
              ...
              return (
                <View key={field.key} className={`px-4 py-4 ${i < FIELDS.length - 1 ? "border-b border-gray-100" : ""}`}>
                  ...
                  <View className="flex-row items-start gap-3">
                    <Icon size={16} color="#9ca3af" style={{ marginTop: field.multiline ? 2 : 0 }} />
                    <TextInput
