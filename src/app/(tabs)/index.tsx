...
const RECENT = [
  { id: "1", name: "BMW 5 Series", price: fmt(32000000), bgColor: "#fffbeb", iconColor: "#b45309", Icon: Car, type: "vehicle" },
  { id: "2", name: "Office Complex VI", price: fmt(200000000), bgColor: "#f0f9ff", iconColor: "#0284c7", Icon: Home, type: "real_estate" },
];
    ...
    <SafeAreaView className="flex-1 bg-gray-50">
      <ScrollView showsVerticalScrollIndicator={false}>
        <View className="pb-6">
          ...
          <View className="px-5 py-3">
            <View className="relative rounded-2xl overflow-hidden h-40 flex-row" style={{ backgroundColor: "#1e1b4b" }}>
              <View className="flex-1 p-5 justify-center gap-1.5">
                ...
                <TouchableOpacity className="bg-amber-400 self-start px-3.5 py-1.5 rounded-xl mt-1">
              ...
              <View className="absolute right-14 -top-5 w-20 h-20 rounded-full" style={{ backgroundColor: "rgba(255,255,255,0.05)" }} />
            </View>
          ...
          <View className="mt-5">
            <View className="px-5">
              <SectionHeader title="Hot Sales" onSeeAll={() => {}} />
            </View>
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              <View className="flex-row gap-3.5 px-5">
                {HOT_SALES.map((item) => (
                  <ProductCard key={item.id} item={item} onPress={goToProduct} variant="vertical" />
                ))}
              </View>
            </ScrollView>
          </View>
          ...
          <View className="mt-7 px-5">
            ...
            <View className="flex-row gap-3.5">
              {RECENT.map((item) => (
                <ProductCard key={item.id} item={item} onPress={goToProduct} variant="horizontal" />
              ))}
            </View>
          </View>
        </View>
